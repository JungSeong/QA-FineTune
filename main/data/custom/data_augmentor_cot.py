...
class CoTAsyncDataAugmentor:
    def __init__(self, model_id, config):
        ...
        self.output_file = getattr(config, 'AUGMENTED_COT_DATA_PATH')
        ...
        for item in tasks:
            if mode == "initial":
                ...
                for target_label in self.labels :
                    ...
                    system_content = LIBRARY_QA_SYSTEM_PROMPT_COT_FEW_SHOT.replace("{target_label}", target_label)
                ...
                for target_label in target_labels :
                    ...
                    system_content = LIBRARY_QA_TARGETED_SYSTEM_PROMPT_COT_FEW_SHOT.replace("{target_label}", target_label)
                    user_content = LIBRARY_QA_USER_TEMPLATE.replace("{faq_content}", faq_context)
                    messages = [
                        {"role": "system", "content": system_content},
                        {"role": "user", "content": user_content}
